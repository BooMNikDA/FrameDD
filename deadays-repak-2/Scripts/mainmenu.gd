...
func create_mod_button(folder_name: String, folder_path: String, preview_path: String, config: Dictionary):
	...
	normal_style.bg_color = Color(0.169, 0.173, 0.169, 0.902)
	normal_style.set_border_width_all(4)
	normal_style.border_color = Color(0.3, 0.3, 1)  # Синий для модов
	button.add_theme_stylebox_override("normal", normal_style)
...
func _on_mod_activate_pressed():
	...
	print("ModLoader найден? ", mod_loader != null)
	
	if mod_loader == null:
		print("ModLoader не найден в /root/ModLoader")
	
		mod_loader = Engine.get_singleton("ModLoader")
		print("ModLoader через Engine? ", mod_loader != null)
		
		if mod_loader == null:
			print("ModLoader не найден вообще!")
			return
	
	print("ModLoader получен")
	
	print("▶Вызываю activate_mod()...")
